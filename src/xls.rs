    ...
    fn parse_workbook(&mut self) -> Result<(), XlsError> {
        ...
        let mut draw_group: Vec<u8> = Vec::new();
        {
            ...
            for record in records {
                ...
                match r.typ {
                    // 2.4.117 FilePass
                    0x002F if read_u16(r.data) != 0 => {break;} // return Err(XlsError::Password),
                    // CodePage
                    0x0042 => {
                    ...
                    0x0017 => {
                        ...
                        xtis.extend(r.data[2..].chunks(6).take(cxti).map(|xti| 
                            {
                                if xti.len() >=4 {
                                    let isup_book = read_u16(&xti[..2]);
                                    let itab_first = read_i16(&xti[2..4]);
                                    let itab_last = read_i16(&xti[4..]);
                                    
                                    Xti {
                                        _isup_book: isup_book,
                                        itab_first: itab_first,
                                        _itab_last: itab_last,
                                    }
                                } else {
                                    let isup_book = read_u16(&xti[..2]);
                                    Xti {
